...
export class Raycaster {
    static step(origin, direction, chunkManager, maxDistance = 8) {
        ...
        let tMaxX = stepX > 0 ? (x + 1 - origin[0]) * tDeltaX : (origin[0] - x) * tDeltaX;
        let tMaxY = stepY > 0 ? (y + 1 - origin[1]) * tDeltaY : (origin[1] - y) * tDeltaY;
        let tMaxZ = stepZ > 0 ? (z + 1 - origin[2]) * tDeltaZ : (origin[2] - z) * tDeltaZ;
